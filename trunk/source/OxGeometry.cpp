...
template<typename T> inline triangle<T,3> make_triangle(const T& _x, const T& _y, const T& _z, const T& _u, const T& _v, const T& _w, const T& _a, const T& _b, const T& _c) {
    triangle<T,3> _triangle;
...
template<typename T> inline bool intersect(const triangle<T,3>& _triangle, const sphere<T,3>& _sphere) {
...
template<typename T> inline point<T,3> mirror(const plane<T,3>& _plane, const point<T,3>& _point) {
    return _point - _plane.normal * (T(2.0) * (dot_product(_plane.normal, _point) - _plane.constant));
}

template<typename T> inline vector<T,3> mirror(const plane<T,3>& _plane, const vector<T,3>& _vector) {
